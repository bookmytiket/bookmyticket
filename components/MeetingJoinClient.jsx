            ...
            <div className="relative z-10 w-full max-w-5xl px-6 py-4 pointer-events-none">
                <div className="pointer-events-auto bg-white/[0.98] backdrop-blur-3xl rounded-[2.5rem] shadow-[0_32px_80px_-20px_rgba(0,0,0,0.6)] border border-white/40 overflow-hidden transform transition-all duration-1000 ease-in-out flex flex-col max-h-[92vh]">
                    ...
                    <div className="px-8 py-4 border-b border-slate-100/60 bg-slate-50/50 flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            ...
                            <div className="min-w-0">
                                <h1 className="text-lg font-black text-slate-900 tracking-tight leading-tight truncate max-w-md">
                                    {eventDetails.title}
                                </h1>
                                <div className="flex items-center gap-2">
                                    <ShieldCheck size={10} className="text-blue-500" />
                                    <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Secure Session</span>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="flex-1 overflow-y-auto no-scrollbar grid grid-cols-1 lg:grid-cols-12 gap-0">
                        ...
                        <div className="lg:col-span-12 xl:col-span-7 p-8 border-r border-slate-100/60 flex flex-col justify-center bg-white/40">
                            <div className="relative w-full max-w-xl aspect-video bg-[#0a0f1e] rounded-[1.5rem] overflow-hidden shadow-2xl border-4 border-white ring-1 ring-slate-100/50 group mx-auto">
                                ...
                                <div className="absolute inset-0 bg-gradient-to-br from-[#1a233a] to-[#0a0f1e] flex flex-col items-center justify-center text-center p-6">
                                    ...
                                    <p className="text-slate-500 font-bold text-[9px] tracking-[0.2em] uppercase">Ready for session</p>
                                </div>

                                {/* Security Badges Overlay - Compact */}
                                <div className="absolute bottom-4 left-4 right-4 flex justify-between items-center bg-black/20 backdrop-blur-md px-4 py-2 rounded-xl border border-white/10">
                                    <div className="flex items-center gap-2">
                                        <div className="w-1.5 h-1.5 bg-emerald-500 rounded-full" />
                                        <span className="text-[8px] font-black text-white uppercase tracking-widest">Camera Ready</span>
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <div className="w-1.5 h-1.5 bg-blue-500 rounded-full shadow-[0_0_8px_rgba(59,130,246,0.8)]" />
                                        <span className="text-[8px] font-black text-white uppercase tracking-widest">HD Link</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="mt-6 max-w-md mx-auto w-full">
                                {status === "success" && meetingStatus === "live" ? (
                                    <div className="space-y-4">
                                        <button 
                                            onClick={handleJoin}
                                            className="w-full py-4 bg-slate-900 hover:bg-blue-600 text-white rounded-[1.5rem] font-black text-xs tracking-[0.2em] uppercase transition-all flex items-center justify-center gap-3 shadow-2xl hover:shadow-blue-500/30 active:scale-95 group"
                                        >
                                            <Video className="w-5 h-5" />
                                            Enter Meeting
                                            {type === "external" && <ExternalLink className="w-3.5 h-3.5 opacity-50 group-hover:opacity-100 transition-opacity" />}
                                        </button>
                                        <div className="flex items-center justify-center gap-3 opacity-30">
                                            <div className="h-[1px] w-6 bg-slate-900" />
                                            <p className="text-[9px] font-black uppercase tracking-[0.1em]">Verified Session Access</p>
                                            <div className="h-[1px] w-6 bg-slate-900" />
                                        </div>
                                    </div>
                                ) : status === "not_booked" ? (
                                    <div className="space-y-4">
                                        <div className="flex items-center gap-3 p-4 bg-amber-50 rounded-2xl border border-amber-100/50">
                                            ...
                                            <div>
                                                <h4 className="font-black text-amber-900 uppercase tracking-widest text-[10px]">Pass Required</h4>
                                                <p className="text-amber-800/70 text-[9px] font-bold uppercase tracking-tight">Ticket needed for access.</p>
                                            </div>
                                        </div>
                                        <button 
                                            onClick={handleBook}
                                            className="w-full py-4 bg-slate-900 hover:bg-amber-600 text-white rounded-[1.5rem] font-black text-xs tracking-[0.2em] uppercase transition-all flex items-center justify-center gap-3 shadow-2xl active:scale-95"
                                        >
                                    ...
                                    <div className="text-center p-6 bg-slate-50/50 rounded-2xl border border-slate-100">
                                        ...
                                        <h3 className="text-base font-black text-slate-400 uppercase tracking-tighter mb-1">Session Hold</h3>
                                        <p className="text-slate-400 text-[9px] font-black uppercase tracking-[0.1em]">Event {meetingStatus === "expired" ? "expired" : "not started"}</p>
                                        <Link href="/meeting/join" className="inline-block mt-4 text-[9px] font-black text-blue-600 uppercase tracking-widest border-b-2 border-blue-600 pb-1">
                                            Return to Portal
                                        </Link>
                                    </div>
                                )}
                        ...
                        {/* RIGHT: Event Details & Meta - Compact */}
                        <div className="lg:col-span-12 xl:col-span-5 p-8 bg-slate-50/30 flex flex-col">
                            <div className="flex-1 space-y-6">
                                <div className="space-y-4">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <p className="text-[8px] font-black text-slate-400 uppercase tracking-[0.2em]">Scheduled Date</p>
                                            <p className="text-sm font-black text-slate-900 tracking-tight">{eventDetails.date}</p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <p className="text-[8px] font-black text-slate-400 uppercase tracking-[0.2em]">Start Time</p>
                                            <p className="text-sm font-black text-slate-900 tracking-tight">{eventDetails.time}</p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="pt-6 border-t border-slate-100">
                                    <h4 className="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] mb-2 flex items-center gap-2">
                                        <Zap className="w-3 h-3 text-amber-500" /> Briefing Agenda
                                    </h4>
                                    <p className="text-[12px] text-slate-600 leading-relaxed font-medium italic underline decoration-slate-200 decoration-[4px] underline-offset-4">
                                        {eventDetails.description || "Virtual session details provided upon entry."}
                                    </p>
                                </div>
                            </div>

                            {/* Security Footer Card - Slimmed */}
                            <div className="mt-8 bg-gradient-to-br from-[#1a233a] to-[#0a0f1e] rounded-[2rem] p-6 text-white relative overflow-hidden shadow-2xl">
                                <div className="absolute -top-12 -right-12 w-32 h-32 bg-blue-600/10 rounded-full blur-[40px]" />
                                <div className="relative z-10">
                                    <div className="flex items-center gap-3 mb-4">
                                        <div className="w-8 h-8 bg-white/10 rounded-lg flex items-center justify-center border border-white/10">
                                            <ShieldCheck className="w-4 h-4 text-emerald-400" />
                                        </div>
                                        <h4 className="font-black text-[11px] uppercase tracking-widest leading-none">Security Protocol</h4>
                                    </div>
                                    <ul className="space-y-2.5">
                                        {["End-to-End Encrypted", "Verified Only", "HD Stream"].map((text, i) => (
                                            <li key={i} className="flex items-center gap-2 text-[9px] font-black uppercase tracking-[0.1em] text-white/70">
