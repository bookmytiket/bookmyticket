...
import { Check, X, Eye, Trash2, ShieldCheck, CheckCircle, Search, Mail, Phone, Building2, Briefcase } from "lucide-react";
import { useToast } from "@/context/ToastContext";

export default function AdminServiceRequestsTable({ t }) {
    ...
    const fetchRequests = useCallback(async () => {
        ...
        try {
            ...
            setRequests((data || []).map(r => ({
                ...
                service_category: r.service_category || r.category,
                description: r.description || r.remarks,
                status: r.status === 'Pending' ? 'Pending Review' : r.status
            })));
        } catch (err) {
            console.error("Fetch Service Requests Error:", err);
            showToast("Failed to fetch service requests", "error");
        } finally {
            setLoading(false);
        }
    }, [showToast]);
    ...
    useEffect(() => {
        ...
        const channel = supabase
            .channel('admin-service-req-sync')
    ...
    const handleUpdateStatus = async (id, status) => {
        try {
            ...
            const res = await fetch('/api/admin/professional-services/action', {
                method: 'POST',
                headers: {
            ...
            showToast(status === 'Approved' ? "Request Approved & Account Created" : `Status updated to ${status}`, 'success');
    ...
    const filteredRequests = useMemo(() => {
        return requests.filter(req => {
            const matchesTab = req.status === activeTab || (activeTab === "Pending Review" && req.status === "Pending Review");
        ...
        <div className="flex flex-col gap-8">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div className="flex gap-2 p-1 bg-slate-100/50 rounded-2xl w-fit">
                    <button 
                        ...
                        className={`px-6 py-2.5 rounded-xl text-sm font-bold transition-all flex items-center gap-2 ${activeTab === "Pending Review" ? 'bg-white text-purple-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                    >
                        <ShieldCheck size={16} />
                        Pending Verification
                        {stats.pending > 0 && <span className="bg-purple-600 text-white text-[10px] px-1.5 py-0.5 rounded-full">{stats.pending}</span>}
                    </button>
                    <button 
                ...
                <div className="flex items-center gap-4">
                    <div className="relative">
                        ...
                        <input 
                            ...
                            className="pl-11 pr-4 py-2.5 rounded-xl bg-white border border-slate-200 outline-none focus:ring-4 focus:ring-purple-500/10 focus:border-purple-500 transition-all text-sm font-medium w-64"
                        />
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 gap-4">
                {filteredRequests.length > 0 ? filteredRequests.map((req) => (
                    <div key={req.id} className="card-premium flex flex-col md:flex-row items-center gap-6 group hover:border-purple-500/30 transition-all">
                        <div className="flex-1 min-w-0">
                            ...
                            <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-1">
                                ...
                                <span className="flex items-center gap-1.5 text-xs font-black text-purple-500/70 uppercase tracking-wider">
                                    <Briefcase size={12} /> {req.service_category}
                                </span>
                            </div>
                        </div>
                            ...
                            <div className="flex gap-2">
                                {req.status === "Pending Review" && (
                                    <button 
                                        ...
                                        className="w-10 h-10 rounded-xl bg-purple-600 text-white flex items-center justify-center hover:scale-105 transition-all shadow-lg shadow-purple-500/20"
                    ...
                    <div className="p-20 text-center bg-white rounded-[32px] border-2 border-dashed border-slate-100">
                        ...
                        <p className="text-sm font-medium text-slate-400 max-w-sm mx-auto">New professional service provider requests will appear here.</p>
                    </div>
                )}
            </div>
                ...
                <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-md z-[100] flex items-center justify-center p-6">
                    <div className="premium-glass max-w-md w-full rounded-[32px] overflow-hidden shadow-2xl animate-in fade-in zoom-in duration-300">
                        <div className="bg-gradient-to-br from-purple-500 to-indigo-600 p-8 text-white relative">
                            ...
                            <h2 className="text-2xl font-black mb-1">Verify Service Provider</h2>
                            <p className="text-white/70 text-sm font-medium">Approve and create account for {selectedRequest.business_name}</p>
                        </div>
                        <div className="p-8 space-y-6">
                            <div className="bg-emerald-50 p-4 rounded-2xl flex gap-3 items-center">
                                <CheckCircle className="text-emerald-500" size={18} />
                                <p className="text-[11px] font-bold text-emerald-700 leading-tight">This will approve the request, create an auth account, and send an invite email.</p>
                            </div>
                            <button 
