    ...
    useEffect(() => {
        ...
        supabase.from('bookings').select('*, events(*)').eq('user_id', uid)
            .then(({ data }) => setEventBookingsList((data || []).map(b => ({
                ...
                eventDate: b.events?.event_start_date || b.events?.startDate || b.events?.event_date || b.events?.date,
                eventTime: b.events?.event_start_time || b.events?.startTime || b.events?.event_time || b.events?.time || "TBA",
