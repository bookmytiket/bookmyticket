-- SUPER PREMIUM DYNAMIC EMAIL SYSTEM
-- High-fidelity templates with Neon Gradients, Glassmorphism, and Interactive Layouts

-- 1. OTP VERIFICATION (Super Premium)
UPDATE public.email_templates
SET subject = '{{otp}} is your BookMyTicket Secret Code 🔐',
    body = '<!DOCTYPE html>
<html>
<head>
    ...
    <style>
        ...
        .header::after { content: ""; position: absolute; bottom: 0; left: 0; right: 0; height: 100px; background: linear-gradient(to top, #ffffff, transparent); }
...
-- 2. BOOKING CONFIRMATION (Super Premium)
UPDATE public.email_templates
SET subject = 'Your Ticket is Ready! 🎟️ {{eventName}}',
    body = '<!DOCTYPE html>
<html>
<head>
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
...
-- 3. PAYMENT SUCCESS (New Template)
INSERT INTO public.email_templates (identifier, name, subject, body, category, auto_send)
VALUES 
('payment_success', 'Customer: Payment Successful', 'Payment Successful! ₹{{amount}} 💰', '<!DOCTYPE html>
<html>
<head>
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link href="https://fonts.googleapis.com/css2?family=Outfit:wght@400;600;800&display=swap" rel="stylesheet">
    <style>
        ...
        .wrapper { max-width: 600px; margin: 40px auto; padding: 0 20px; }
        .card { background: #ffffff; border-radius: 32px; overflow: hidden; box-shadow: 0 30px 60px rgba(0,0,0,0.08); border: 1px solid #f1f5f9; }
        .success-badge { background: #ecfdf5; color: #059669; padding: 10px 20px; border-radius: 100px; display: inline-block; font-weight: 800; font-size: 12px; text-transform: uppercase; margin-bottom: 20px; }
        .amount-hero { font-size: 48px; font-weight: 800; color: #0f172a; margin: 20px 0; }
        .invoice-table { width: 100%; border-top: 2px solid #f1f5f9; margin-top: 30px; padding-top: 30px; }
        .invoice-row { display: table; width: 100%; margin-bottom: 15px; }
        .invoice-label { display: table-cell; color: #64748b; font-size: 14px; }
        .invoice-value { display: table-cell; text-align: right; color: #1e293b; font-weight: 700; font-size: 14px; }
    </style>
</head>
<body>
    <div class="wrapper">
        <div class="card" style="padding: 50px 40px; text-align: center;">
            <div class="success-badge">Payment Received</div>
            <h2 style="margin: 0; color: #64748b; font-size: 16px;">Transaction Successful</h2>
            <div class="amount-hero">₹{{amount}}</div>
            <p style="color: #94a3b8; font-size: 14px;">Transaction ID: {{transactionId}}</p>
            
            <div class="invoice-table">
                <div class="invoice-row">
                    <span class="invoice-label">Booking Reference</span>
                    <span class="invoice-value">#{{bookingId}}</span>
                </div>
                <div class="invoice-row">
                    <span class="invoice-label">Base Amount</span>
                    <span class="invoice-value">₹{{baseAmount}}</span>
                </div>
                <div class="invoice-row">
                    <span class="invoice-label">Platform Fee</span>
                    <span class="invoice-value">₹{{platformFee}}</span>
                </div>
                <div class="invoice-row" style="margin-top: 20px; border-top: 1px solid #f1f5f9; padding-top: 20px;">
                    <span class="invoice-label" style="font-weight: 800; color: #0f172a;">Total Paid</span>
                    <span class="invoice-value" style="font-size: 20px; color: #ec4899;">₹{{amount}}</span>
                </div>
            </div>
            
            <p style="margin-top: 40px; font-size: 13px; color: #94a3b8;">A copy of your invoice has been saved to your account.</p>
        </div>
    </div>
</body>
</html>', 'Payment', true)
ON CONFLICT (identifier) DO UPDATE SET body = EXCLUDED.body, subject = EXCLUDED.subject;
