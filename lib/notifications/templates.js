...
function interpolate(templateString, payload) {
    ...
    // Inject global defaults that are commonly used in templates
    const mergedPayload = {
        site_url: process.env.NEXT_PUBLIC_BASE_URL || 'https://bookmyticket.net',
        support_email: process.env.NEXT_PUBLIC_SUPPORT_EMAIL || 'support@bookmyticket.net',
        ...payload
    };

    return templateString.replace(/\{\{([^}]+)\}\}/g, (match, key) => {
        const value = mergedPayload[key.trim()];
        return value !== undefined ? value : ''; // Replace with empty string if not found, rather than leaving {{...}} which breaks links
    });
}
