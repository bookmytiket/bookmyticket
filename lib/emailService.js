...
import { parseEmailTemplate } from './templateParser.js';
...
export const sendTemplatedEmail = async ({ templateIdentifier, to, variables = {}, metadata = {} }) => {
  ...
  try {
    ...
    const { data: template, error: templateError } = await supabaseAdmin
      ...
      .select('*')
      .eq('template_key', templateIdentifier)
      .single();
    ...
    const subject = parseEmailTemplate(template.subject_template, variables);
    const body = parseEmailTemplate(template.html_content, variables);
    ...
    await supabaseAdmin.from('email_logs').insert({
      recipient_email: to,
      subject,
      template_key: templateIdentifier,
