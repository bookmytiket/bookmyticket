...
export async function reverseGeocode(lat, lng) {
  // Try official JS SDK Geocoder first if available
  if (typeof window !== "undefined" && window.google && window.google.maps && window.google.maps.Geocoder) {
    try {
      const geocoder = new window.google.maps.Geocoder();
      const response = await geocoder.geocode({ location: { lat, lng } });
      if (response.results && response.results.length > 0) {
        return parseGoogleAddressComponents(response.results);
      }
    } catch (err) {
      console.warn("[GoogleMaps SDK] Reverse geocoding failed, falling back to REST/Nominatim:", err);
    }
  }

  const apiKey = GOOGLE_MAPS_API_KEY;
  if (!apiKey || apiKey === "YOUR_GOOGLE_MAPS_API_KEY_HERE") {
    console.warn("[GoogleMaps] API key not set. Falling back to Nominatim.");
  ...
  if (typeof window !== "undefined" && window.google && window.google.maps && window.google.maps.Geocoder) {
    try {
      ...
      if (response.results && response.results.length > 0) {
        ...
        const geo = parseGoogleAddressComponents(response.results);
        geo.lat = first.geometry.location.lat();
        geo.lng = first.geometry.location.lng();
        return geo;
      }
    } catch (err) {
      console.warn("[GoogleMaps SDK] Geocoding failed, falling back to REST:", err);
    }
  }

  const apiKey = GOOGLE_MAPS_API_KEY;
  if (!apiKey || apiKey === "YOUR_GOOGLE_MAPS_API_KEY_HERE") {
    console.warn("[GoogleMaps] API key not set for geocode.");
    return null;
  }
